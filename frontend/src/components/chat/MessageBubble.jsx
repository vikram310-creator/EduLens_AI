...
export default function MessageBubble({ message, index }) {
  ...
  // Show user's real initial instead of hardcoded 'U'
        ...
        {isUser ? (
              ...
              <div className={`p-2 ${message.images.length > 1 ? 'grid grid-cols-2 gap-1.5' : ''}`}
                ...
                {message.images.map((img, i) => (
                  ...
                  >
                    ...
                    <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, height: '36px', background: 'linear-gradient(to top, rgba(0,0,0,0.5), transparent)', borderRadius: '0 0 10px 10px' }} />
                    {img.name && (
                      <span className="absolute bottom-1.5 left-2 text-white/70 opacity-0 group-hover:opacity-100 transition-opacity" style={{ fontSize: '9px', fontWeight: 500 }}>
                        {img.name}
                      </span>
                    )}
                  </div>
          ...
          <div className="relative w-full">
            ...
            <div className="mt-2.5 flex items-center gap-3">
              ...
              {/* Copy — shown on hover via group, always in flow so no z-index issues */}
              <button
                onClick={copyMsg}
